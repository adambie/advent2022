...
fn read_blueprints() -> Vec<Blueprint> {
    ...
    for line in lines {
        ...
        max_ore_use = max_ore_use.max(obsidian_robot_cost_ore.max(geode_robot_cost_ore));
        blueprints.push(
                    Blueprint {
                        ore_robot_cost_ore,
                        clay_robot_cost_ore,
...
fn find_best_config(mut inventory: Inventory, bleuprint: &Blueprint, mut turns_left:i32 ) -> i32 {
    ...
    if turns_left > 0 {
        ...
        for robot in [Collector::Ore, Collector::Clay, Collector::Obsidian, Collector::Geode] {
            ...
            if inventory.construct == Collector::Geode && inventory.obsidian_robots == 0 {
                continue;
            }
            if inventory.construct == Collector::Obsidian && inventory.clay_robots == 0 {
                continue;
            }            
            if inventory.construct == Collector::Clay && 
                bleuprint.obsidian_robot_cost_clay ==  inventory.clay_robots{
                continue;
            }
            if inventory.construct == Collector::Obsidian && 
                bleuprint.geode_robot_cost_obsidian ==  inventory.obsidian_robots {
                continue;
            }
            if inventory.construct == Collector::Ore && 
            bleuprint.max_ore_use ==  inventory.ore_robots {
                continue;
            }
            
            let result = find_best_config(inventory.clone(), &bleuprint, turns_left);
